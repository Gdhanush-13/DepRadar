...
Public browser and API scans are limited to 10 attempts per client IP per 10-minute rolling window by default. A `429` response includes `Retry-After`. The single Render instance processes one scan at a time, limits metadata lookups to six at once, caps each scan at 100 dependencies, and rejects metadata responses over 5 MB to keep memory bounded. The limiter is intentionally in-process; use a shared central store before running multiple app instances.
